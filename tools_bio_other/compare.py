...
anno = (
    ...
    'Diff: %s (D/U: %.2f%%)  (A:%s  B:%s)\n' % (
        ...
        len(diff_B)) +
    'A: %s \nB: %s' % (out1, out2))
print(anno0 + '\n' + anno, file=open('venn_%s--VS--%s.stat.txt' % (out1, out2), 'w'))
plt.title(anno0)
...
# %%
plt.savefig('venn_%s--VS--%s.pdf' % (out1, out2), dpi=200, bbox_inches='tight')
plt.savefig('venn_%s--VS--%s.png' % (out1, out2), dpi=200, bbox_inches='tight')
plt.savefig('venn_%s--VS--%s.svg' % (out1, out2), dpi=200, bbox_inches='tight')

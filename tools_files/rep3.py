...
# @Last Modified time: 2019-04-23 16:23:09
...
def replace_list(Largv):
    ...
    for line in fi_list:
        ...
        Dlist[Lline[0].strip()] = Lline[1].strip()
    print('>>> 已建立索引, 即将开始替换...')
    ...
    for line in fi:
        ...
        Lline = line.split('\t')
        Lline = [x.strip() for x in Lline]
        Lline[Num] = [x.strip() for x in Lline[Num].split(', ')]
        for i_tmp, key in enumerate(Lline[Num]):
            if key:
                keylast = key
                try:
                    keyValue = Dlist[key]
                    Lline[Num][i_tmp] = keyValue
                except KeyError:
                    try:
                        keyValue = Dlist[key.replace('_', ':')]
                        Lline[Num][i_tmp] = keyValue
                        keylast = key.replace('_', ':')
                    except KeyError:
                        Neer += 1
                        print('WARNING %s: 跳过第 %s 行替换，列表中值 %s 不存在' % (Neer, Nrow, key))
                        # print(Dlist)
                        # print(Lline[Num])
                        # sys.exit()
                        continue
        Lline[Num] = ', '.join(Lline[Num])
        fo.write('\t'.join(Lline) + '\n')
    try:
        print('>>> 替换完毕! 最后一次替换是将"%s"替换为"%s"\n进行了%d次替换，请查看新文件: %s' %
              (key, Dlist[keylast], Nrow, new_name))
    except KeyError:

...
alias q=vjob
# 进程
alias pp="ps ux|head -1;ps ux|grep -v PID|sort -rn -k +3|head -20"  # 查看占用资源最多的进程
...
###  工具  ###
alias send="python ${tools_path}/sendmail.py ${mymail}"  # 发送邮件。可在搜狗输入法将邮箱等常用短语自定义添加，如输入qqqq会默认把邮箱放在输入法第一位。设置在: 搜狗输入法-->高级-->候选拓展-->自定义短语
...
alias cag="iconv -f gbk -t utf-8 "  # 输出gbk文件
# 2) 用于返回当前文件(夹)绝对路径

...
# @Last Modified time: 2019-03-14 16:50:21
...
def main():
    path = 'out_svg'
    L = os.listdir(path)
    L = [x for x in L if x.endswith('tre.svg')]
    ...
    # print(Dbiao)
    fo = open('%s.table-svg.html' % path, 'w')
    fo.write('<html>\n\t<body>\n')
    for biao in Lbiao:
        Llines = Dbiao[biao]
        Lwuzhong = sorted({x[1] for x in Llines})
        Lshuju = sorted({re.findall('bestTree.(.*?).tre.pep.tre.svg', x[3])[0]
                         for x in Llines})
        ...
        for wuzhong in Lwuzhong:
            ...
            for x in Lshuju:
                file = '_'.join([biao, wuzhong,
                                 'RAxML_bestTree.%s.tre.pep.tre.svg' % x])

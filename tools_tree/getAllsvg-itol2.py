...
# @Last Modified time: 2019-03-22 10:50:58
...
def fmain(path):
    ...
    try:
        ...
        # 读取信息1, xls
        D_xls = {}
        # print('/'.join(Lpath[:4])+ '/result.xls.xls')
        # sys.exit()
        try:
        ...
        # itol.fmain(path, 'out_svg/' + name_svg, {'display_mode': 1})
        ...
        for gene_ID in quchong(L_cankao):
            ...
            data = data.replace('>%s</text' % gene_ID,
                                ' fill="red" >%s</text' % gene_ID)
        for gene_ID in quchong(Lfilter):
            data = data.replace('>%s</text' % gene_ID,
                                ' fill="green" >%s</text' % gene_ID)

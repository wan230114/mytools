...
# @Last Modified time: 2019-08-31 10:35:04
...
def fargv():
    ...
    parser.add_argument('-a', '--afters', type=str, default=False,
                        help='输入jobnames(多个用英文逗号分隔)，提取输入的job节点之后预执行的所有job')
    parser.add_argument('-b', '--breaking', type=str, default=False,
                        help='输入jobnames(多个用英文逗号分隔)，去除输入的job节点之后预执行的所有job')
...
def fmain(fipath, keyword, viewname, sortjob, afters, breaking):
    '''主程序'''
    ...
    if breaking:
        ...
        iters = [x for x in iters if x not in iters_tmp]

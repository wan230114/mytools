...
def fargv():
    parser = argparse.ArgumentParser(
        ...
        epilog=('注意事项：\n'
                ...
                '    当-k中的job与-d同时存在时，会自动去除-k的job\n\n'
                '当前版本：\n'
                '  v1.7 修改使用场景\n'
                '       -b参数改为-d参数，由原来的“去除自身后面的job(不包含自身)”\n'
                '                           修改为“去除自身后面的job(包含自身)”\n'
                '       -b参数改为，以某个节点为提取的终止节点，提取包括自身及之前的所有job\n'
                '历史版本：\n'
    ...
    parser.add_argument('-b', '--breakjob', type=str, default=False,
                        help='输入jobnames(多个用英文逗号分隔)，提取输入的job节点之前预执行的所有job，包含jobnames在内')
    parser.add_argument('-d', '--deljob', type=str, default=False,
                        help='输入jobnames(多个用英文逗号分隔)，去除输入的job节点之后预执行的所有job，包含jobnames在内')
...
def get_before_job(iters, D_orders):
    '''***函数需要开发***
    处理节点之前的分支的函数
    '''
    iters2 = []
...
def fmain(fipath, keyword, viewname, sortjob, afters, breakjob, deljob):
    '''主程序'''
    ...
    if breakjob:
        L_breakjob = breakjob.split(',')
        Lcheck.append(L_breakjob)
    if deljob:
        L_deljob = deljob.split(',')
        Lcheck.append(L_breakjob)
    # **** 标签 ****
    # 去重
    L_tmp = []
    ...
    if breakjob:
        iters_tmp = get_afters_job(L_breakjob, D_orders)
        iters_tmp = [x for x in iters_tmp if x not in L_breakjob]  # 保留终止的节点

...
# @Last Modified time: 2019-08-29 20:40:44
...
def fargv():
    parser = argparse.ArgumentParser(
        ...
        description=('本程序用于格式化处理sjm语法文件，可提取指定job，还可按运行顺序对job排序\n'
                     '    当前版本：\n'
                     '      v1.4 加入功能，能提取某一节点之后还需要执行的所有job，添加为-a参数\n'
                     '    历史版本：\n'
    ...
    parser.add_argument('-a', '--afters', action='store_true',
                        default=False,
                        help='是否提取某一个job节点之后会执行的所有job')
...
def fmain(fipath, keyword, viewname, sortjob, afters):
    '''主程序'''
    ...
    if afters:
        iters = get_afters_job(iters, D_orders)
...
def main():
    ...
    # sys.argv = ['', './test/sjms/s2-2.job', '-k', 'PASA4training', '-as']

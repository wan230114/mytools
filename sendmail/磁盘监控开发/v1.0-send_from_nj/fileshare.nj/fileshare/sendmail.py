...
# @Last Modified time: 2019-03-02 21:38:53
...
if __name__ == "__main__":
    ...
    # Largv = ["", "--help"]
    with open('sendmail.txt', 'rb') as fi:
    ...
    # print(s1, s2)
    # print(s1, s2)
    if data.replace(s1, '').replace(s2, '').split():
        Ldata = data.split()
        # print(Ldata)
        if Ldata[Ldata.index('天津：') + 3] == '南京：':
            data = data.replace(s1, '').strip()
        if Ldata[Ldata.index('南京：') + 2] == Ldata[len(Ldata) - 1]:
            data = data.replace(s2, '').strip()
        # print(Ldata[Ldata.index('南京：') + 2])
        if data:
            #data = data
            data = 'hi, all,\n检测到集群剩余磁盘空间低于5T，以下为不足5T的磁盘详情:\n' + data
            ...
            os.system(
                "echo start at time `date +%F'  '%H:%M` 'The data is empty, not send.' >>sendmail.py.log")
